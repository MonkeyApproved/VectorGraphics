...
import { TokenType } from './equationParsing/tokenUtils/tokenEnums';
import { Token } from './equationParsing/tokenUtils/tokenTypes';
...
function logToken(token: Token): void {
  switch (token.type) {
    ...
    case TokenType.Function:
      console.log(`${pos2String(token)} function: ${token.definition.name}, nArgs: ${token.nArgs}`);
      break;
