...
import { setEquationError } from './equation';
...
export function computeAllResults({ equationId, state }: { equationId: string; state: DataState }) {
  ...
  while (equationReadyForEvaluation !== undefined) {
    ...
    computeEquationResult({ equationId: equationReadyForEvaluation, state });
...
export function computeEquationResult({ equationId, state }: { equationId: string; state: DataState }) {
